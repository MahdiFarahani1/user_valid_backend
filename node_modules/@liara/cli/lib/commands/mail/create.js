...
import { MAIL_SERVICE_MODES, MAIL_SERVICE_PLANS, MAIL_SERVICE_URL, DEV_MODE, MAIL_SERVICE_URL_DEV, } from '../../constants.js';
import { createDebugLogger } from '../../utils/output.js';
class MailCreate extends Command {
    ...
    async run() {
        ...
        try {
            ...
            if (error.response && error.response.statusCode === 409) {
                this.error(`The mail server already exists. Please use a unique name for your mail server.`);
            }
            if (error.response &&
                error.response.statusCode === 403 &&
                error.response.body) {
                this.error(`You are allowed to create only one Mail Server on the free plan.`);
            }
...
MailCreate.description = 'create a mail server';
MailCreate.flags = {
    ...Command.flags,
    domain: Flags.string({
        description: 'domain',
...
MailCreate.aliases = ['mail:create'];
