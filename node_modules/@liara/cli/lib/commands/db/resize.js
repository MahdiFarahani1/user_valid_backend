            ...
            const { plan } = (await inquirer.prompt({
                ...
                choices: [
                    ...Object.keys(plans.databases)
                        .filter((plan) => {
                        if (plans.databases[plan].available &&
                        ...
                        return {
                            value: plan,
                            name: `RAM: ${ram}${spacing(5, ram)}GB,  CPU: ${cpu}${spacing(5, cpu)}Core,  Disk: ${disk}${spacing(3, disk) + 'GB'}${storageClass || 'SSD'},  Price: ${price.toLocaleString()}${price ? spacing(7, price) + 'Tomans/Month' : ''}`,
                        };
                    }),
                ],
            }));
...
Resize.description = 'resize a database';
Resize.PATH = 'v1/databases/{database-id}/resize';
Resize.flags = {
    ...Command.flags,
    name: Flags.string({
        char: 'n',
