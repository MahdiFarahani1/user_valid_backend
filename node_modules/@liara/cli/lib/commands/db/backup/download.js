    ...
    async run() {
        ...
        try {
            ...
            const downloadLink = await this.got
                .post(BackUp.PATH.replace('{database-id}', databaseID) +
                `/${backupName}/download`)
                .json();
            this.log(`download link: ${downloadLink.link}`);
            const _ = backupName.split('/');
            const outputDefault = _[_.length - 1];
            const output = flags.output || (await this.promptOutput('./' + outputDefault));
            if (output.length > 0) {
...
Please open up https://console.liara.ir/databases and create the database, first.`);
    ...
    async promptOutput(defaultName) {
...
BackUp.description = 'download a database backup';
BackUp.PATH = 'v1/databases/{database-id}/backups';
BackUp.aliases = ['db:backup:dl'];
BackUp.flags = {
    ...Command.flags,
    name: Flags.string({
        char: 'n',
