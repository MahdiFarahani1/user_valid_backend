...
import { OBJECT_STORAGE_API_URL_DEV, OBJECT_STORAGE_API_URL, OBJ_PERMISSION, DEV_MODE, } from '../../constants.js';
import { createDebugLogger } from '../../utils/output.js';
import spacing from '../../utils/spacing.js';
class BucketCreate extends Command {
    async setGotConfig(config, isObjMode = true) {
        if (isObjMode) {
            ...
            const { plan } = (await inquirer.prompt({
                ...
                choices: [
                    ...Object.keys(plans.objectStorage)
                        ...
                        return {
                            value: plan,
                            name: `Space: ${space}${spacing(5, space)}GB,  Storage Class: ${storageClass}${spacing(5, storageClass)},  Price: ${price.toLocaleString()}${price ? spacing(7, price) + 'Tomans/Month' : ''}`,
                        };
                    }),
                ],
            }));
...
BucketCreate.description = 'create a bucket';
BucketCreate.flags = {
    ...Command.flags,
    name: Flags.string({
        description: 'name',
...
BucketCreate.aliases = ['bucket:create'];
