...
class Deploy extends Command {
    async run() {
        ...
        config.buildCache = !(config['no-cache'] || ((_b = config.build) === null || _b === void 0 ? void 0 : _b.cache) === false);
        this.debug(`Using Build Cache: ${config.buildCache ? 'Enabled' : 'Disabled'}`);
        config.dockerfile = config.dockerfile || ((_c = config.build) === null || _c === void 0 ? void 0 : _c.dockerfile);
        if (config.dockerfile) {
            this.debug(`Using Custom Dockerfile: ${config.dockerfile}`);
        }
        config['build-location'] =
            config['build-location'] || ((_d = config.build) === null || _d === void 0 ? void 0 : _d.location);
        if (config['build-location']) {
            this.debug(`Declared build location: ${config['build-location']}`);
        }
        const buildArgs = ((_e = config.build) === null || _e === void 0 ? void 0 : _e.args) || config['build-arg'];
        if (Array.isArray(buildArgs)) {
            const firstPriority = buildArgsParser(config['build-arg'] || []);
            const secondPriority = buildArgsParser(((_f = config.build) === null || _f === void 0 ? void 0 : _f.args) || []);
            // @ts-ignore
        ...
        try {
            ...
            if (flags.detach) {
                ...
                this.log();
                const { project } = await this.got(`v1/projects/${config.app}`).json();
                const defaultSubdomain = config.region === 'iran' && !Boolean(project.network)
                    ? '.iran.liara.run'
                    : '.liara.run';
                ...
                const { domains } = await this.got(`v1/domains?project=${config.app}`).json();
...
Please open up https://console.liara.ir/apps and unfreeze the app.`;
            ...
            if (error.response && error.response.statusCode === 408) {
                const message = "Oops! It seems like there's a disruption, and the request has timed out (Error 408). Please check your internet connection and try again.";
                return this.error(message);
            }
            if (error.response &&
                error.response.statusCode >= 400 &&
...
Additionally, you can also retry the build with the debug flag:
        ...
        try {
            await buildLogs(this.got, releaseID, isCanceled, (output) => {
                ...
                if (output.state === 'PUSHING') {
                    ...
                    if (output.line) {
                        let outputString = '';
                        const [progressbar, layersCounter, imageSize] = output.line.split('-'); // progressbar, layers counter and image size separated by '-'
                        outputString = `(layer ${layersCounter}) Pushing: total size ${bytes(parseInt(imageSize))} ${progressbar}`;
                        this.spinner.clear().frame();
    ...
    validateDeploymentConfig(config) {
        ...
        if (((_b = config.build) === null || _b === void 0 ? void 0 : _b.cache) && typeof config.build.cache !== 'boolean') {
